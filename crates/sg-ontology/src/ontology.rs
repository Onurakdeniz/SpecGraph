...
fn endpoint_types(edge_type: &str) -> Option<(&'static [&'static str], &'static [&'static str])> {
    match edge_type {
        ...
        "MERGE_ACCEPTS_GRAPH_MERGE" => Some((&["GitMerge"], &["GraphMerge"])),
        "SPEC_HAS_VALIDATION_RUN" => Some((&["Spec"], &["ValidationRun"])),
        "SPEC_HAS_PULL_REQUEST" => Some((&["Spec"], &["PullRequest"])),
        "SPEC_HAS_RELEASE" => Some((&["Spec"], &["Release"])),
        "SPEC_HAS_MERGE" => Some((&["Spec"], &["GitMerge", "GraphMerge"])),
        ...
        "RELEASE_HAS_SNAPSHOT" => Some((&["Release"], &["GraphSnapshot"])),
        "RELEASE_HAS_ARTIFACT" => Some((&["Release"], &["ReleaseArtifact"])),
        "RELEASE_HAS_CHECKSUM" => Some((&["Release"], &["ArtifactChecksum"])),
        "RELEASE_HAS_EVIDENCE" => Some((&["Release"], &["ReleaseEvidence"])),
        "ARTIFACT_HAS_CHECKSUM" => Some((&["ReleaseArtifact"], &["ArtifactChecksum"])),
        "ACTION_REQUIRES_VALIDATION_RECIPE" => Some((&["ActionNode"], &["ValidationRecipe"])),
        "COMMIT_PLAN_REQUIRES_VALIDATION_RECIPE" => Some((&["CommitPlan"], &["ValidationRecipe"])),
        "VALIDATION_RECIPE_HAS_COMMAND" => Some((&["ValidationRecipe"], &["ValidationCommand"])),
        "VALIDATION_RUN_SATISFIES_RECIPE" => Some((&["ValidationRun"], &["ValidationRecipe"])),
        "VALIDATION_RUN_HAS_BUILD" => Some((&["ValidationRun"], &["BuildRun"])),
        "VALIDATION_RUN_HAS_TYPECHECK" => Some((&["ValidationRun"], &["TypecheckRun"])),
        "VALIDATION_RUN_HAS_LINT" => Some((&["ValidationRun"], &["LintRun"])),
        "VALIDATION_RUN_HAS_FORMAT_CHECK" => Some((&["ValidationRun"], &["FormatCheck"])),
        "SPEC_HAS_TEST_INTENT" => Some((&["Spec"], &["TestIntent"])),
        "ACCEPTANCE_CRITERION_HAS_TEST_INTENT" => Some((&["AcceptanceCriterion"], &["TestIntent"])),
        "TEST_INTENT_HAS_ASSERTION" => Some((&["TestIntent"], &["TestAssertion"])),
        "TEST_INTENT_HAS_POSITIVE_CASE" => Some((&["TestIntent"], &["PositiveCase"])),
        "TEST_INTENT_HAS_NEGATIVE_CASE" => Some((&["TestIntent"], &["NegativeCase"])),
        "TEST_INTENT_HAS_REGRESSION_CASE" => Some((&["TestIntent"], &["RegressionCase"])),
        "TEST_INTENT_HAS_SECURITY_CASE" => Some((&["TestIntent"], &["SecurityCase"])),
        "TEST_INTENT_VALIDATED_BY" => Some((&["TestIntent"], &["TestCase", "TestRun"])),
        "SPEC_HAS_REVIEW" => Some((&["Spec"], &["Review"])),
        "ACTION_HAS_REVIEW" => Some((&["ActionNode"], &["Review"])),
        "PR_HAS_REVIEW" => Some((&["PullRequest"], &["Review"])),
        "REVIEW_HAS_COMMENT" => Some((&["Review"], &["ReviewComment"])),
        "REVIEW_REQUESTS_CHANGE" => Some((&["Review"], &["RequestedChange"])),
        "REQUESTED_CHANGE_RESOLVED_BY" => Some((&["RequestedChange"], &["ReviewResolution"])),
        "REQUESTED_CHANGE_APPROVED_BY" => Some((&["RequestedChange"], &["ReviewApproval"])),
        "RELEASE_HAS_ROLLOUT_PLAN" => Some((&["Release"], &["RolloutPlan"])),
        "ROLLOUT_USES_FEATURE_FLAG" => Some((&["RolloutPlan"], &["FeatureFlag"])),
        "RELEASE_HAS_ROLLBACK_STRATEGY" => Some((&["Release"], &["RollbackStrategy"])),
        "RELEASE_HAS_POST_RELEASE_CHECK" => Some((&["Release"], &["PostReleaseCheck"])),
        "RELEASE_HAS_HEALTH_CHECK" => Some((&["Release"], &["ReleaseHealthCheck"])),
        "RELEASE_OBSERVES_METRIC" => Some((&["Release"], &["Metric"])),
        "RELEASE_OBSERVES_LOG_EVENT" => Some((&["Release"], &["LogEvent"])),
        "RELEASE_OBSERVES_TRACE_SPAN" => Some((&["Release"], &["TraceSpan"])),
        "RELEASE_HAS_AUDIT_EVENT" => Some((&["Release"], &["AuditEvent"])),
        "RELEASE_HAS_OPERATIONAL_ALERT" => Some((&["Release"], &["OperationalAlert"])),
        "RELEASE_HAS_SLO" => Some((&["Release"], &["SLO"])),
        "POST_RELEASE_CHECK_CREATED_ISSUE" => {
            Some((&["PostReleaseCheck", "ReleaseHealthCheck"], &["Issue"]))
        }
        "POST_RELEASE_CHECK_TRIGGERED_ROLLBACK" => Some((
            &["PostReleaseCheck", "ReleaseHealthCheck"],
            &["RollbackStrategy"],
        )),
        "POST_RELEASE_CHECK_REQUIRES_REPLAN" => {
            Some((&["PostReleaseCheck", "ReleaseHealthCheck"], &["ActionNode"]))
        }
