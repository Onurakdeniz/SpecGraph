...
- Runtime, domain graph, adapter/observation, server, and SDK Rust crates now own their implementation or direct owning-crate re-exports; `sg-core` is only a compatibility facade.
- `sg-cli`, `sg-server`, and `sg-sdk` depend on owning crates directly instead of depending on `sg-core`.
...
- `sg-core` remains a compatibility facade for older Rust imports.
...
- Keep `sg-core` as compatibility re-exports only; new code should depend on the owning crate directly.
