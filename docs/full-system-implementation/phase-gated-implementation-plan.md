...
| 5.8 | Workspace modularization | 01, 45, 46, 48 | `refactor: modularize workspace crates` | Trusted core, runtime/domain crates, adapter/observation crates, server/SDK boundaries, and Studio package boundaries exist with dependency checks; Rust implementation is physically extracted into owning crates and `sg-core` is compatibility re-exports only. |
