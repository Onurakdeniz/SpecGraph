...
When a repository has `.specgraph`, `sg release evidence` includes replayed graph `stateHash`, `lastSequence`, and `lastEventId`. In clean source-only releases where `.specgraph` is absent, the evidence still records that graph snapshot binding is required by release policy and must be supplied by the release project graph.
